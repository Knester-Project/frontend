...
export default function Create() {
    ...
    return (
        <>
            ...
            {isOpen && (
                <div className="z-50 fixed inset-0 flex justify-center items-center bg-black/50 p-2 overflow-auto">
                    <section className="bg-background py-4 border border-border rounded-2xl w-full max-w-3xl max-h-[85vh] overflow-auto">
                        ...
                        <form onSubmit={handleSubmit(onSubmit)}>
                                ...
                                <AnimatePresence mode="wait">
                                    ...
                                    {step === 1 && (
                                        <motion.div key="step1" variants={stepVariants} initial="hidden" animate="visible" exit="exit" className="space-y-4">
                                            ...
                                            <StateSelect disabled={newPost.isPending} value={selectedState} onChange={(v) => setValue("location.state", v, { shouldValidate: true })} error={errors.location?.state?.message} />

                                            <ZodInput disabled={newPost.isPending} type="text" register={register} name="location.city" label="City" required placeholder="Enter City" />
                                            {errors.location?.city && <ErrorText message={errors.location.city.message} />}

                                            <ZodInput disabled={newPost.isPending} type="text" register={register} name="location.town" label="Town" required placeholder="Enter Town" />
                                            {errors.location?.town && <ErrorText message={errors.location.town.message} />}

                                            <ZodInput disabled={newPost.isPending} type="text" register={register} name="location.street" label="Street (Optional)" placeholder="Enter Street" />
                                            {errors.location?.street && <ErrorText message={errors.location.street.message} />}

                                            <div className="flex justify-end">
                                                <Button disabled={newPost.isPending} type="button" onClick={() => setStep(2)}>Next</Button>
                                            </div>
                                        </motion.div>
                                    )}
                                    ...
                                    {step === 2 && (
                                        <motion.div key="step2" variants={stepVariants} initial="hidden" animate="visible" exit="exit" className="space-y-4">
                                            ...
                                            <div className="flex justify-between">
                                                <Button disabled={newPost.isPending} type="button" variant="outline" onClick={() => setStep(1)}>Back</Button>
                                                <Button disabled={newPost.isPending} type="button" onClick={() => setStep(3)}>Next</Button>
                                            </div>
                                        </motion.div>
                                    )}
                                    ...
                                    {step === 3 && (
                                        <motion.div key="step3" variants={stepVariants} initial="hidden" animate="visible" exit="exit" className="space-y-4">
                                            ...
                                            <ZodInput disabled={newPost.isPending} type="text" register={register} name="fullName" label="Full Name" required placeholder="Enter offender's full name" />
                                            ...
                                            <div className="space-y-2 max-h-80 overflow-y-auto">
                                                ...
                                                {phoneNumbers.map((_: string, index: number) => (
                                                    <div key={index}>
                                                        <ZodInput disabled={newPost.isPending} type="tel" register={register} name={`phoneNumbers.${index}`} placeholder="Enter phone number" required={index === 0} />
                                            ...
                                            <div className="space-y-3 overflow-y-auto">
                                                {socialMedia.map((_, index) => (
                                                    <div key={index} className="relative space-y-3 bg-accent/60 dark:bg-accent/20 p-3 border rounded-xl">
                                                        ...
                                                        <ZodInput disabled={newPost.isPending} type="text" register={register} name={`socialMedia.${index}.platform`} label="Platform" required placeholder="Instagram" />
                                                        <ZodInput disabled={newPost.isPending} type="text" register={register} name={`socialMedia.${index}.username`} label="Username" required placeholder="@username" />
                                                        <ZodInput disabled={newPost.isPending} type="url" register={register} name={`socialMedia.${index}.profileLink`} label="Profile Link (Optional)" placeholder="https://..." />
                                                    </div>
                                                ))}
                                            ...
                                            <div className="flex justify-between mt-4">
                                                <Button disabled={newPost.isPending} type="button" variant="outline" onClick={() => setStep(2)}>Back</Button>
                                                <Button disabled={newPost.isPending} type="button" onClick={() => setStep(4)}>Next</Button>
                                            </div>
                                        </motion.div>
                                    )}
                                    ...
                                    {step === 4 && (
                                        <motion.div key="step4" variants={stepVariants} initial="hidden" animate="visible" exit="exit">
                                            ...
                                            <div className="mt-10">
                                                <FileUploader multiple disabled={newPost.isPending} value={files} onChange={setFiles} />
                                            ...
                                            <div className="flex justify-end mt-4">
                                                <Button type="button" disabled={newPost.isPending} variant="outline" onClick={() => setStep(3)}>Back</Button>
                                            </div>
                                        </motion.div>
                                    )}
                                </AnimatePresence>
                            ...
                            <div className={`${(step === 4 && files.length > 0) ? "block" : "hidden"} mb-4 px-4`}>
                                <Button type="submit" className="mt-1 w-full" disabled={newPost.isPending}>
